...
export const UpdateUserPreferencesSchema = z.object({
  ...
  daily_digest_time: z.string().regex(/^([01]\d|2[0-3]):([0-5]\d)$/, { message: 'Formato de hora inválido (HH:mm)' }).optional(),
  weekly_digest_enabled: z.boolean().optional(),
  weekly_digest_day: z.string().optional(),
  weekly_digest_time: z.string().regex(/^([01]\d|2[0-3]):([0-5]\d)$/, { message: 'Formato de hora inválido (HH:mm)' }).optional(),
