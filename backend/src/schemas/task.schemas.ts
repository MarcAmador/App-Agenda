...
export const CreateTaskSchema = z.object({
  ...
  start_time:   z.string().regex(/^\d{2}:\d{2}(:\d{2})?$/, 'Formato de hora inválido (HH:MM)').nullable().optional(),
  end_time:     z.string().regex(/^\d{2}:\d{2}(:\d{2})?$/, 'Formato de hora inválido (HH:MM)').nullable().optional(),
  location:     z.string().max(255).nullable().optional(),
  scope_period: TaskScopeEnum.default('semanal'),
  category:     z.string().max(100).nullable().optional(),
  tags:         z.array(z.string().max(50)).default([]),
  participants: z.array(z.string().max(100)).default([]),
  materials:    z.array(z.string().max(200)).default([]),
